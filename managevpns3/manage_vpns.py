...
#        sdk_vars["tenant_str"] = "".join(x for x in cgx_session.tenant_name if x.isalnum()).lower()
    ...
    while loop:
        ...
        if selected_action == 'edit_sitelista':
            site_list_a = sites.edit_site_list(site_list_a, "Site List A", site_name_list, sdk_vars["tenant_str"], site_tags)
        elif selected_action == 'edit_sitelistb':
            site_list_b = sites.edit_site_list(site_list_b, "Site List B", site_name_list, sdk_vars["tenant_str"], site_tags)
